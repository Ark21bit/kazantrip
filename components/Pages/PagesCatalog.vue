		...
		<CatalogContainer :type="type" class="wrapper" :timetable="true">
            <ModalRequestIndividual #default="{ openModal }">
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-5 lg:gap-y-7.5 h-fit max-lg:(pb-7.5 border-b border-#EBEBEB)">
					<div v-if="pageInfo?.lang_info?.visible_text" class="text-sm leading-1.4 col-span-full" v-html="pageInfo?.lang_info?.visible_text"></div>
                    <CardCatalog v-for="card in type === 'timetable' ? pageInfo : pageInfo?.info_products?.data" :nameplates="[card?.lang_info?.visible_text]?.filter(a => a !== undefined)" @order="card?.type_id === 12 ? openModal(card?.lang_info?.title) : openOrder(card?.slug), reachGoal('acquire_from_listing')" :key="card?.id"
                        ...
                        :category_id="card?.category_id" :is-radio-gid="card?.is_radio_gid" :price-old="card?.price_see_old" :rating="card?.rating" :nearest_timetables_info="card.nearest_timetables_info"/>
                </div>
