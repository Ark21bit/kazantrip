<template>
    ...
    <div class="col-span-full mt-7.5 lg:mt-10 pb-7.5 border-b border-#EBEBEB">
        <div class="wrapper flex flex-col gap-5 lg:gap-7.5 items-start">
            <h1 class="text-6.5 lg:text-9.5 leading-1.2 font-bold font-Montserrat text-diamondBlack max-w-262">{{ pageInfo?.lang_info?.title }}</h1>
    ...
    <div class="mt-7.5 grid lg:grid-cols-[1fr_1.03fr_1fr] rounded-5 ring-(inset 1 #F6F6F6) shadow-base">
        <div class="p-5 lg:p-6 border-b lg:border-r border-#EBEBEB flex flex-col justify-center gap-1.5">
            <p class="text-sm leading-1.2 text-diamondBlack">{{ generalConfig?.static_info?.global_words?.nearest_date }}</p>
            <p class="text-2xl font-semibold leading-1.2 text-primary">{{ nearestDate }}</p>
        </div>
        <div class="p-5 lg:p-6 border-b lg:border-r border-#EBEBEB flex flex-col justify-center gap-1.5">
            <p class="text-sm leading-1.2 text-diamondBlack">{{ generalConfig?.static_info?.global_words?.price_list }}</p>
            <p class="text-2xl font-semibold leading-1.2 text-primary">{{ pageInfo?.price_see }}₽<span class="ml-1.5 leading-1.2 text-#A4A4A4 text-sm line-through decoration-diamondBlack">{{ pageInfo?.price_see_old }}₽</span></p>
        </div>
    ...
    <div class="mt-15 lg:mt-22">
        ...
        <div class="mt-5 lg:mt-7.5 grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
            <div class="ring-(inset 1 #F6F6F6) rounded-5 py-4 px-3 lg:p-5 bg-white shadow-base">
                ...
                <p class="mt-1.5 lg:mt-.75 font-semibold text-diamondBlack leading-1.2 lg:leading-1.2 text-base lg:text-2xl">{{ nearestDate }}</p>
            </div>
            <div class="ring-(inset 1 #F6F6F6) rounded-5 py-4 px-3 lg:p-5 bg-white shadow-base">
                ...
                <p class="mt-1.5 lg:mt-.75 font-semibold text-diamondBlack leading-1.2 lg:leading-1.2 text-base lg:text-2xl">{{ pageInfo?.duration_event_text }}</p>
            </div>
            <div class="ring-(inset 1 #F6F6F6) rounded-5 py-4 px-3 lg:p-5 bg-white shadow-base">
                ...
                <p class="mt-1.5 lg:mt-.75 font-semibold text-diamondBlack leading-1.2 lg:leading-1.2 text-base lg:text-2xl">{{ getTitleCategoriesProduct(pageInfo?.category_id) }}</p>
            </div>
            <div class="ring-(inset 1 #F6F6F6) rounded-5 py-4 px-3 lg:p-5 bg-white shadow-base">
                ...
                <p class="mt-1.5 lg:mt-.75 font-semibold text-diamondBlack leading-1.2 lg:leading-1.2 text-base lg:text-2xl"> {{ pageInfo?.is_radio_gid ? 'Да (бесплатно)' : 'Нет' }}</p>
            </div>
            <div class="ring-(inset 1 #F6F6F6) rounded-5 bg-white shadow-base col-span-full lg:col-span-3">
                ...
                <div class="flex flex-col lg:flex-row lg:border-t border-#EBEBEB">
                    <div v-for="item in pageInfo?.start_place_types" class="flex-1 py-3 lg:py-5 p-5 border-t lg:border-r lg:last:border-r-none border-#EBEBEB">
                        ...
                        <p class="mt-1.5 lg:mt-.75 font-semibold text-diamondBlack leading-1.2 lg:leading-1.2 text-base lg:text-2xl">{{ getTitlePlacesStart(item) }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mt-10 lg:mt-17.5">
        <h2 class="text-diamondBlack text-5.5 lg:text-7 font-bold leading-1.2 lg:leading-1.2 font-Montserrat">{{ generalConfig?.static_info?.global_words?.all_price }}</h2>
        <div class="grid relative grid-cols-[repeat(auto-fit,minmax(122px,1fr))] mt-5 lg:mt-7.5 p-3.75 py-5 lg:px-5 lg:py-6 gap-y-5 gap-4 lg:gap-5 bg-#E2F1F2 rounded-5">
            <div v-for="ticket in pageInfo?.info_prices?.data" class="flex flex-col gap-1.5 lg:gap-2 p-4 lg:p-5 rounded-5 ring-(1 inset #F6F6F6) font-semibold bg-white">
                <p class="text-diamondBlack text-sm lg:text-base leading-1.4 lg:leading-1.4">{{ getTitleTypeTicket(ticket?.type_ticket_id) }}</p>
                <p class="text-primary text-2xl lg:text-8 leading-1.25 lg:leading-1.25 flex flex-col items-end lg:flex-row gap-x-1.5">{{ ticket?.price }}₽<span class="line-through lg:mb-2.5 text-#A4A4A4 leading-1.2 decoration-diamondBlack text-sm">1350₽</span></p>
            </div>
    ...
    <div class="mt-15 lg:mt-25">
        <h2 class="text-diamondBlack text-5.5 lg:text-7 font-bold leading-1.2 lg:leading-1.2 font-Montserrat">{{ generalConfig?.static_info?.global_words?.included_in_the_price }}</h2>
