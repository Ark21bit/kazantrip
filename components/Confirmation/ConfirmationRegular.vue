<template>
    ...
    <div class="mt-7.5 lg:mt-10 shadow-base rounded-5 ring ring-#F6F6F6 ring-inset lg:py-2.5">
        ...
        <div v-for="item in filterTickets" class="flex flex-col lg:flex-row justify-between gap-3 py-3 px-5 lg:px-7.5 last:border-none border-b border-#E8E8E8 text-sm leading-1.4 text-diamondBlack">
            <p>{{ getTitleTypeTicket(item?.id) }}</p>
            <p class="font-medium">({{ item?.count }}) {{ item?.price * item?.count }} руб.</p>
        </div>
        <div v-for="item in filterAdditionals" class="flex flex-col lg:flex-row justify-between gap-3 py-3 px-5 lg:px-7.5 last:border-none border-b border-#E8E8E8 text-sm leading-1.4 text-diamondBlack">
            <p>{{ item?.title }}</p>
            <p class="font-medium">({{ item?.count }}) {{ item?.price * item?.count }} руб.</p>
        </div>
        <div class="flex flex-col lg:flex-row justify-between gap-3 py-3 px-5 lg:px-7.5 last:border-none border-b border-#E8E8E8 text-sm leading-1.4 text-diamondBlack">
