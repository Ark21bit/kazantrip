<template>
    <FeedbackFormContainer :subTitle="generalConfig?.static_info?.global_words?.tell_about_impressions_trip" :title="generalConfig?.static_info?.global_words?.send_review" img="/imgs/reviewForm.png">
        <FormKit id="create-review" @submit="reviewStore" v-model="forms" type="form" form-class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-4.5" :actions="false">
            <FormKit name="rating" :value="5" size="text-6.5" validation="required" :label="generalConfig?.static_info?.global_words?.you_mark" type="ratingC" input-class="max-lg:text-sm" />
            ...
            <FormKit name="email" validation="email|required|length:0,64" :validation-label="generalConfig?.static_info?.global_words?.email_en" :placeholder="generalConfig?.static_info?.global_words?.email_en" type="email" input-class="max-lg:text-sm" />
        ...
        <Modal @close="closeModal" size="md" class="flex flex-col items-center" :is-show="isModalShow">
