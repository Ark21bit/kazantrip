<template>
    <FeedbackFormContainer img="/imgs/feedback.png" class="mt-25 lg:mt-37.5">
        <FormKit :id="id" v-model="forms" @submit="statementsStore" type="form" form-class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-4.5" :actions="false">
            ...
            <FormKit name="phone" :validation="[['matches', /^[0-9\(\)\-\+\s]+$/], ['required']]" v-maska:[options] type="tel" :validation-label="generalConfig?.static_info?.global_words?.number_telephone" :placeholder="generalConfig?.static_info?.global_words?.number_telephone" />
