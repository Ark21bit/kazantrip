            ...
            <template v-for="link in generalConfig?.static_info?.menu?.header">
                <Dropdown v-if="link?.children?.length ?? 0 > 0" class="py-4 -mx-3.75 px-3.75 border-b border-#EBEBEB">
                    <template #dropdown-header-content>
                        <CustomLink :title="link?.title_hidden" :to="link?.slug">{{ link?.title }}</CustomLink>
                    </template>
                    <CustomLink v-for="item in link?.children" :title="item?.title_hidden" :to="item?.slug" class="text-fblack text-3.75 leading-1.25">{{ item?.title }}</CustomLink>
                </Dropdown>
                <CustomLink v-else :to="link?.slug" :title="link?.title_hidden" class="py-4 -mx-3.75 px-3.75 border-b border-#EBEBEB text-fblack text-3.75 leading-1.25 hover:text-primary">{{ link?.title }}</CustomLink>
            </template>
        </div>
