<template>
    <FormKit @submit="setOrder" v-model="forms" type="form" form-class="flex flex-col gap-5 lg:gap-3 3xl:gap-6 lg:mt-1" :actions="false">
        ...
        <div class="flex flex-col gap-4.5 lg:gap-3 3xl:gap-5">
            ...
            <FormKit name="date" validation="required" type="datepickerC" :minDate="$dayjs().tz('Europe/Moscow').toDate()" :validation-label="generalConfig?.static_info?.global_words?.date" :placeholder="generalConfig?.static_info?.global_words?.date"></FormKit>
        </div>
        ...
        <div class="flex flex-col gap-4 lg:gap-3 3xl:gap-5 max-lg:mt-2.5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-3 3xl:gap-4.5">
                ...
                <FormKit name="client_telephone" :validation="getPhoneValidation()" type="selectTel" :validation-label="generalConfig?.static_info?.global_words?.telephone" />
