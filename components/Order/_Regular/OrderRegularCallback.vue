<template>
    <template v-if="status === 'success'">
        <ModalThanksContent @ok="emit('close')" :subTitle="generalConfig?.static_info?.global_words?.waiting_call_manager" :title="generalConfig?.static_info?.global_words?.congratulations" />
    </template>
    <template v-else>
        <FormKit @submit="statementsStore" v-model="forms" type="form" form-class="flex flex-col gap-6" :actions="false">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-4.5">
                <FormKit name="name_service" :value="name" :disabled="true" validation="required:trim" type="text" :validation-label="generalConfig?.static_info?.global_words?.title_excursion" :placeholder="generalConfig?.static_info?.global_words?.title_excursion" outer-class="col-span-full" />
                ...
                <FormKit name="email" validation="email|required|length:0,64" type="email" :validation-label="generalConfig?.static_info?.global_words?.email_en" :placeholder="generalConfig?.static_info?.global_words?.email_en" />
                <FormKit name="phone" :validation="getPhoneValidation()" v-maska:[options] type="tel" :validation-label="generalConfig?.static_info?.global_words?.telephone" :placeholder="generalConfig?.static_info?.global_words?.telephone" />
            ...
            <div class="flex flex-col lg:flex-row gap-3 lg:justify-between">
                ...
                <Button :disabled="status === 'pending'" :loading="status === 'pending'" @click="emit('close')" class="w-full lg:w-49" type="button" variant="outline">{{ generalConfig?.static_info?.global_words?.cancel }}</Button>
            </div>
        </FormKit>
    </template>
</template>
