<template>
    ...
    <template v-else>
        <FormKit @submit="statementsStore" v-model="forms" :config="theme === 'green' ? { innerClass: 'bg-white' } : {}" type="form" form-class="flex flex-col gap-6" :actions="false">
            ...
            <div class="flex flex-col lg:flex-row gap-3 lg:justify-between">
                ...
                <Button v-if="closeIsHidden" :disabled="status === 'pending'" :loading="status === 'pending'" @click="emit('close')" class="w-full lg:w-49" type="button" variant="outline">{{ generalConfig?.static_info?.global_words?.cancel }}</Button>
            </div>
        </FormKit>
    </template>
</template>
