<template>
    <div v-bind="$attrs" class="flex flex-col gap-7.5 lg:gap-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 lg:gap-y-7.5">
            <CardCatalog v-for="card in products" @order="openOrder(card?.slug)" :type_id="card?.type_id" :img="card?.media_preview" :slug="card?.slug" :title="card?.lang_info?.title" :duration="card?.duration_event" :price="card?.price_see" :description="card?.lang_info?.description"
