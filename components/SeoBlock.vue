<template>
    <div class="flex flex-col gap-5 gap-7.5">
        <div v-for="item in isShort ? dataShorts : data" class="flex flex-col gap-5 lg:gap-7.5">
            <div class="text-5.5 lg:text-2xl font-semibold font-Montserrat leading-1.2 lg:leading-1.2 text-fblack" v-html="item?.lang_info?.title"></div>
            <div class="text-sm lg:text-base leading-1.4 lg:leading-1.4 text-fblack" v-html="item?.lang_info?.text"></div>
        </div>
