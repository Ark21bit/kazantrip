<template>
    <ModalRequestIndividual #default="{ openModal }">
        <Swiper class="w-full flex flex-col lg:flex-row relative contain-layout overflow-visible" v-bind="options, $attrs">
            <SwiperSlide class="flex flex-col">
                <h3 class="max-lg:hidden text-7 font-bold leading-1.2 font-Montserrat text-fblack [&>span]:text-primary"><span>Индивидуальная</span> экскурсия</h3>
                <div class="lg:mt-7.5 flex flex-col gap-5 p-5 lg:pr-22 lg:p-6 last:mr-0 mr-5 lg:bg-[url(/imgs/pattern.svg)] bg-[position:top_0_right_24px] grow bg-repeat-y shadow-base rounded-2xl ring-(1 inset #F6F6F6)">
                    ...
                    <Button @click="openModal(undefined), reachGoal('acquire-private')" class="w-full lg:w-41 mt-auto" size="sm">{{ generalConfig?.static_info?.global_words?.order }}</Button>
                </div>
            </SwiperSlide>
            <SwiperSlide class="flex flex-col">
                ...
                <div class="lg:mt-7.5 flex flex-col gap-5 p-5 lg:pr-22 lg:p-6 last:mr-0 mr-5 lg:bg-[url(/imgs/pattern.svg)] bg-[position:top_0_right_24px] grow bg-repeat-y shadow-base rounded-2xl ring-(1 inset #F6F6F6)">
                    ...
                    <p class="text-sm lg:text-base text-second leading-1.4 lg:leading-1.4 [&>a]:text-primary">По вопросам организации многодневного тура, включающего экскурсию "{{ title }}" обращайтесь по телефону <a href="tel:+79370093234">+7 937 009 32 34</a> и электронной почте
                        <a href="mailto:ex@kazantrip.ru">ex@kazantrip.ru</a>
                    </p>
                    <Button @click="openModal(undefined), reachGoal('acquire-tour')" class="w-full lg:w-41 mt-auto" size="sm">{{ generalConfig?.static_info?.global_words?.order }}</Button>
                </div>
            </SwiperSlide>
