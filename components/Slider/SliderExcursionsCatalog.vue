<template>
	<div v-if="links?.length" class="flex flex-col gap-5 lg:flex-row relative contain-layout">
...
const currentPage = computed(() => {
	const { id = null, parent_id = null } = generalConfig.value?.structure.find(a => a.slug.slice(0, 1) === '/' ? a.slug === route.path : `/${a.slug}` === route.path) ?? {}
	return { id, parent_id }
})

const links = computed(() => {
	if (route.path === localePath('/excursions') || route.path.includes(localePath('/excursion/timetable'))) return generalConfig.value?.structure
	if (currentPage.value.id === null && currentPage.value.parent_id === null) return []
	return generalConfig.value?.structure.filter(a => a.parent_id === currentPage.value.id || a.parent_id === currentPage.value.parent_id)
})
</script>