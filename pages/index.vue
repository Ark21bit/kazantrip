<template>
    ...
    <div class="flex flex-col lg:flex-row gap-5 mt-7.5 lg:mt-12.5 justify-between">
        ...
        <div class="flex flex-col lg:flex-row gap-2.5 lg:gap-5 max-lg:border-t border-#E2E2E2 max-lg:pt-5">
            <FormKit type="date" outer-class="max-lg:w-full"></FormKit>
            <Button class="max-lg:w-full">Подобрать экскусрию</Button>
        </div>
    </div>
    <SplideExcursions class="py-5 mt-10"></SplideExcursions>
    <div class="flex flex-col gap-8 mt-20">
        <h2 class="font-Montserrat text-8 leading-1.2 font-bold text-diamondBlack">Доступные экскурсии</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-7.5">
            <CardCatalog v-for="n in 10"></CardCatalog>
            <Button class="col-span-full mx-auto">Больше экскурсий</Button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mt-15 lg:mt-37.5">
        <div class="flex flex-col gap-3 min-h-48.75 p-5.75 pr-20 border-(#F6F6F6 1) bg-[position:top_0_right_30px] bg-[url(/imgs/pattern.svg)] bg-repeat-y rounded-5">
            <h3 class="text-diamondBlack text-6.5 font-Montserrat font-bold leading-1.2"><span class="text-#FF3D15">Яндекс</span> отзывы</h3>
        ...
        <div class="flex flex-col gap-3 min-h-48.75 p-5.75 border-(#F6F6F6 1) bg1 bg-cover bg-center bg-black/50 rounded-5">
            <h3 class="text-white text-6.5 font-Montserrat font-bold leading-1.2">Эксклюзивная экскурсия в закулисье цирка!</h3>
            <Button size="sm" class="w-41 mt-auto" color="yellow">Подробнее</Button>
        </div>
        <div class="flex flex-col gap-3 min-h-48.75 p-5.75 pr-20 border-(#F6F6F6 1) bg-[position:top_0_right_30px] bg-[url(/imgs/pattern.svg)] bg-repeat-y rounded-5">
            <h3 class="text-diamondBlack text-6.5 font-Montserrat font-bold leading-1.2">Индивидуальные эксркусии</h3>
            <Button size="sm" class="w-41 mt-auto">Подробнее</Button>
        </div>
    </div>
    <div class="mt-20 lg:mt-37.5">
        <h2 class="text-8 leading-1.2 font-bold font-Montserrat text-diamondBlack wrapper">Наши партнеры</h2>
        <SplidePartners class="mt-10" />
    </div>
</template>
