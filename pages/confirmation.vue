            ...
            <component class="mt-5 3xl:mt-10 shadow-base rounded-5 ring ring-#F6F6F6 ring-inset 3xl:py-2.5" :is="confirmationComponent">
                ...
                <template #head>
                    <div class="flex flex-col gap-2 3xl:gap-4 items-center text-center px-5 p-3 3xl:pt-7.5 3xl:pb-7.5 border-b border-#E8E8E8">
						...
						<Button class="lg:hidden w-full" size="sm" @click="storeOrder(storeOrderInfo)" :disabled="storeStatus === 'pending'" :loading="storeStatus === 'pending'">{{ generalConfig?.static_info?.global_words?.confirmation_order }}</Button>
                    </div>
                </template>
...
const cancel = () => {
    $reset()
    navigateTo(localePath('/excursions/regulyarnyye'))
}
