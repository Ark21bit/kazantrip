...
export const useToast = () => {
    ...
    const add = (options: ToastAddOptions) => {
        const { title, severity = 'info', messages, duration } = options
        toasts.value.push({ title: title, severity: severity, id: new Date().getTime(), messages: messages, duration:duration })
    }
    const del = (id: number) => {
