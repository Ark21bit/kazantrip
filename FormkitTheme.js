...
export default {
    ...
    selectTel: {
        inner: '$remove:overflow-hidden flex relative items-center border border-#E8E8E8 focus-within:border-primary rounded-lg',
        input: 'items-center px-3.75 py-3.5 appearance-none bg-transparent flex grow justify-between w-full',
        dropdownWrapper: 'absolute mt-1.25 shadow-base rounded-lg ring ring-inset ring-#F6F6F6 z-1 top-full overflow-auto min-w-full max-h-70 scroll-customize',
        listbox: 'bg-white p-2.5',
        listitem: 'flex gap-2 items-center cursor-pointer hover:bg-primary px-3 py-2.5 rounded-lg hover:text-white transition-colors duration-500',
        listitemSelect: 'bg-primary text-white',
        selectIcon: 'text-lg transition-transform duration-500 ease-linear leading-none',
        selectText: 'min-h-1.2em text-base leading-1.2 text-fblack line-clamp-1 text-start',
        placeholder: 'min-h-1.2em text-base leading-1.2 text-#868686 line-clamp-1 text-start',
    ...
    toggleButtons: {
        inner:'$reset',
        options: 'flex flex-wrap gap-2.5',
        option: "rounded-2 bg-#e2f1f2 text-3.5 lg:text-4 font-600 leading-1.2 text-fblack py-3 px-5 hover:(bg-primary text-white) [&.active]:(bg-primary text-white) transition-colors"
    }
}
